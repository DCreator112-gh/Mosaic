...
class MosaicWindow(QWidget):
    ...
    def get_checked(self, widget_id: str) -> bool:
        widget = self.widgets.get(widget_id)
        if isinstance(widget, (QCheckBox, QRadioButton)):
            return widget.isChecked()
        raise ValueError(f"get_checked() must only be called on <input> elements (got {type(widget).__name__})")

    def set_checked(self, widget_id: str, value: bool) -> None:
        widget = self.widgets.get(widget_id)
        if isinstance(widget, (QCheckBox, QRadioButton)):
            widget.setChecked(value)
        else:
            raise ValueError(f"set_checked() must only be called on <input> elements (got {type(widget).__name__})")
    
    def get_value(self, widget_id: str) -> float | int | str | None:
        widget = self.widgets.get(widget_id)
